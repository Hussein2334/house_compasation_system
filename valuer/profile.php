<?php
// valuer/profile.php - Valuer Profile Management
session_start();
...
// Get user statistics for valuer
$stats_query = "SELECT 
    (SELECT COUNT(*) FROM valuations WHERE valuer_id = ?) as total_valuations,
    (SELECT COUNT(*) FROM valuations WHERE valuer_id = ? AND MONTH(created_at) = MONTH(CURRENT_DATE()) AND YEAR(created_at) = YEAR(CURRENT_DATE())) as monthly_valuations,
    (SELECT COALESCE(SUM(total_compensation), 0) FROM valuations WHERE valuer_id = ?) as total_compensation,
    (SELECT COUNT(*) FROM documents d JOIN claims c ON d.claim_id = c.id JOIN valuations v ON c.id = v.claim_id WHERE v.valuer_id = ?) as total_documents,
    (SELECT COUNT(DISTINCT claim_id) FROM valuations WHERE valuer_id = ?) as total_claims_valued";
        ...
        <div class="md:col-span-1">
            <div class="profile-sidebar">
                <div class="profile-avatar">
                    ...
                    <div class="profile-role">Mkaguzi (Valuer)</div>
                </div>
            ...
            <div class="stats-grid mt-4">
                ...
                <div class="stat-card">
                    <div class="stat-number"><?php echo number_format($stats['monthly_valuations'] ?? 0); ?></div>
                    <div class="stat-label">Tathmini za Mwezi</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?php echo number_format($stats['total_claims_valued'] ?? 0); ?></div>
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center justify-between py-2 border-b border-outline-variant">
                                <div>
                                    ...
                                    <div class="text-xs text-secondary">Pokea arifa kuhusu tathmini zilizokabidhiwa</div>
                                </div>
