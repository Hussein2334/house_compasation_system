<?php
// valuer/includes/valuer-sidebar.php - Valuer Sidebar Component with Dropdown Menus (Updated)

// Determine current page for active states
$current_page = basename($_SERVER['PHP_SELF']);
?>

<!-- Valuer Sidebar (Desktop & Mobile Drawer) -->
<aside class="fixed inset-y-0 left-0 w-72 bg-surface-container-lowest border-r border-outline-variant z-50 transform -translate-x-full transition-transform duration-300 ease-in-out md:relative md:translate-x-0 flex flex-col h-full overflow-y-auto custom-scrollbar" id="sidebar">
    ...
    <div class="px-md py-4 mb-2 flex items-center justify-between border-b border-outline-variant shrink-0 bg-surface-container-lowest">
        <a href="dashboard.php" class="flex items-center gap-2">
            ...
            <span class="font-headline-md text-primary font-bold">HCS | Valuer</span>
        </a>
    ...
    <div class="flex-1 px-3 py-2">
        ...
        <div class="flex items-center gap-3 p-3 bg-surface-container-low rounded-xl mb-4">
            <div class="w-10 h-10 bg-primary rounded-full flex items-center justify-center text-white font-bold text-sm shadow-md">
                <?php echo strtoupper(substr($_SESSION['full_name'] ?? 'V', 0, 1)); ?>
            </div>
            <div class="flex-1 min-w-0">
                <p class="font-label-md text-on-surface truncate font-semibold"><?php echo htmlspecialchars($_SESSION['full_name'] ?? 'Valuer'); ?></p>
                <p class="text-[11px] text-secondary">Mkaguzi (Valuer)</p>
            </div>
        </div>
        ...
        <div class="space-y-1">
            ...
            <!-- ==================== DROPDOWN: TATHMINI NA MADAI ==================== -->
            <?php 
            $is_my_valuations_active = ($current_page == 'my-valuations.php');
            ...
            $tathmini_sub_active = ($is_my_valuations_active || $is_valuations_active || $is_claims_active || $is_valuations_reports_active || 
                                   (isset($_GET['status']) && in_array($_GET['status'], ['valuation', 'legal_review'])));
            ?>
            <div class="dropdown-container <?php echo $tathmini_sub_active ? 'dropdown-open' : ''; ?>">
                <button class="w-full flex items-center justify-between px-3 py-2.5 <?php echo $tathmini_sub_active ? 'bg-secondary-container text-on-secondary-container font-semibold' : 'text-secondary hover:bg-surface-container-high'; ?> rounded-xl transition-all duration-200" onclick="toggleDropdown(this)">
                    <div class="flex items-center gap-3">
                        <span class="material-symbols-outlined text-xl">real_estate_agent</span>
                        <span>Tathmini na Madai</span>
                    </div>
                ...
                <div class="dropdown-content pl-11 flex flex-col gap-1 mt-1 ml-2 border-l-2 border-outline-variant">
                    <a href="my-valuations.php" class="py-2.5 text-sm <?php echo $is_my_valuations_active ? 'text-primary font-semibold bg-primary-container/15 rounded-lg px-3' : 'text-secondary hover:text-primary hover:bg-primary-container/10 rounded-lg px-3'; ?> transition-all duration-200">
                        ⭐ Tathmini Zangu
                    </a>
            ...
            <div class="dropdown-container <?php echo $is_reports_active ? 'dropdown-open' : ''; ?>">
                ...
                <div class="dropdown-content pl-11 flex flex-col gap-1 mt-1 ml-2 border-l-2 border-outline-variant">
                    <a href="valuations-reports.php" class="py-2.5 text-sm <?php echo ($current_page == 'valuations-reports.php') ? 'text-primary font-semibold bg-primary-container/15 rounded-lg px-3' : 'text-secondary hover:text-primary hover:bg-primary-container/10 rounded-lg px-3'; ?> transition-all duration-200">
                        📈 Ripoti za Tathmini
                    </a>
