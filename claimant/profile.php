<?php
// claimant/profile.php - Claimant Profile Management
session_start();
...
$stats_query = "SELECT 
    ...
    (SELECT COUNT(*) FROM claims WHERE claimant_id = ? AND status = 'submitted') as pending_claims,
    (SELECT COUNT(*) FROM claims WHERE claimant_id = ? AND status = 'valuation') as valuation_claims,
    (SELECT COUNT(*) FROM claims WHERE claimant_id = ? AND status = 'legal_review') as legal_review_claims,
    (SELECT COUNT(*) FROM claims WHERE claimant_id = ? AND status = 'approved') as approved_claims,
    (SELECT COUNT(*) FROM claims WHERE claimant_id = ? AND status = 'paid') as paid_claims,
    (SELECT COUNT(*) FROM valuations v JOIN claims c ON v.claim_id = c.id WHERE c.claimant_id = ?) as total_valuations,
    (SELECT COUNT(*) FROM payments p JOIN claims c ON p.claim_id = c.id WHERE c.claimant_id = ? AND p.payment_status = 'completed') as total_payments,
    (SELECT COALESCE(SUM(p.amount), 0) FROM payments p JOIN claims c ON p.claim_id = c.id WHERE c.claimant_id = ? AND p.payment_status = 'completed') as total_paid,
    (SELECT COUNT(*) FROM documents d JOIN claims c ON d.claim_id = c.id WHERE c.claimant_id = ?) as total_documents";
$stats_stmt = mysqli_prepare($conn, $stats_query);
mysqli_stmt_bind_param($stats_stmt, "iiiiiiiiii", 
        ...
        <div class="md:col-span-1">
            <div class="profile-sidebar">
                <div class="profile-avatar">
                    ...
                    <div class="profile-role">Mwombaji (Claimant)</div>
                </div>
            ...
            <div class="stats-grid mt-4">
                <div class="stat-card">
                    ...
                    <div class="stat-label">Madai</div>
                </div>
                <div class="stat-card">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-label">Nyaraka</div>
                </div>
            </div>
        </div>
