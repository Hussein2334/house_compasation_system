<?php
// claimant/my-valuations.php - View My Valuations
session_start();
...
// Check if user is logged in - FIXED REDIRECT LOOP
if (!isset($_SESSION['user_id']) || !isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true) {
    header("Location: ../auth/login.php");
    exit();
}

// Check if user is claimant - if not, redirect to appropriate dashboard
...
if (!empty($search_term)) {
    $where_clauses[] = "(c.claim_number LIKE ? OR c.project_name LIKE ?)";
...
if ($summary_stmt) {
    ...
    $summary = ['total_valuations' => 0, 'total_compensation' => 0, 'avg_compensation' => 0];
}
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-secondary text-sm mt-1">Angalia tathmini za mali zako na fidia iliyopendekezwa</p>
        </div>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-label">Jumla ya Fidia Iliyopendekezwa</div>
        </div>
        <div class="stat-card">
    ...
    <div class="filter-bar">
        <form method="GET" action="" class="filter-grid">
            ...
            <div>
                ...
                <input type="text" name="search" class="filter-input" placeholder="Namba ya dai au mradi..." value="<?php echo htmlspecialchars($search_term); ?>">
            </div>
            <div>
            ...
            <table class="valuations-table">
                <thead>
                    <tr>
                        ...
                        <th><a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'project_name', 'order' => $sort_by == 'project_name' && $sort_order == 'ASC' ? 'DESC' : 'ASC'])); ?>" class="hover:text-primary">Mradi</a></th>
                        <th>Aina ya Mali</th>
                        <th>Wilaya</th>
                        <th class="text-right">Thamani ya Mali</th>
                        <th class="text-right">Posho ya Usumbufu</th>
                        <th class="text-right">Posho ya Usafiri</th>
                        <th class="text-right"><a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'total_compensation', 'order' => $sort_by == 'total_compensation' && $sort_order == 'ASC' ? 'DESC' : 'ASC'])); ?>" class="hover:text-primary">Jumla ya Fidia</a></th>
                        <th>Mkaguzi</th>
                        <th><a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'created_at', 'order' => $sort_by == 'created_at' && $sort_order == 'ASC' ? 'DESC' : 'ASC'])); ?>" class="hover:text-primary">Tarehe</a></th>
                        <th>Hali</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="12" class="text-center py-12 text-secondary">
                            ...
                            <div class="text-sm mt-2">Tathmini zitaonekana hapo baada ya kukamilika</div>
                        </td>
                    </tr>
                    <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="font-medium"><?php echo htmlspecialchars($valuation['project_name'] ?? '-'); ?></td>
                        ...
                        <td class="text-sm"><?php echo htmlspecialchars($valuation['valuer_name'] ?? '-'); ?></td>
                        ...
                        <td>
                            <span class="status-badge <?php echo $valuation['claim_status']; ?>">
                                <?php echo getStatusLabel($valuation['claim_status']); ?>
                            </span>
                        </td>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div class="flex items-start gap-3">
            ...
            <div>
                <p class="text-sm font-semibold text-blue-800">Kuhusu Tathmini</p>
                <p class="text-sm text-blue-700 mt-1">Tathmini hufanywa na wakaguzi wetu waliohitimu. Kiasi cha fidia kinapendekezwa kulingana na thamani ya soko ya mali, usumbufu unaosababishwa, na gharama za usafiri. Baada ya kukubaliwa, utapokea malipo kwa mujibu wa ratiba.</p>
            </div>
        </div>
    </div>
